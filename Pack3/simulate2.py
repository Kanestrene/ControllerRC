...
def simulate():
    obstacles = []   # obstáculos fixos, se quiseres adicionar depois
    ...
    for k in range(steps):
        ...
        for i, car in enumerate(cars):
            ...
            state = (car["x"], car["y"], car["yaw"], car["v"], car["w"])

            a_cmd, alpha_cmd, target_idx, car["last_near"], cte = pure_pursuit_control_acc(
                px, py, state,
        ...
        if k % 5 == 0:
            ...
            for car in cars:
                ...
                ax.plot(px[ti], py[ti], "x", markersize=10, label=f"Alvo {car['name']}")
                ax.add_patch(Circle((car["x"], car["y"]), car["_plot"]["Ld"], fill=False))

            ax.set_aspect("equal", "box")
            ax.grid(True)
            ax.set_title("Multi-carro: Pure Pursuit + CBF-QP")
