...
def simulate():
    ...
    # Elipse do robô
    a_ell, b_ell = 0.35, 0.25   # semi-eixos [m]
    margin = 0.01             # margem CBF/desenho
    ...
    for k in range(steps):
        ...
        state = (x, y, yaw, v, w)
        a_cmd, alpha_cmd, target_idx, last_near, cte = pure_pursuit_control_acc(
            px, py, state, last_near_idx=last_near, Ld=2, v_ref=v_ref
        )
        
        # -------- CBF-QP (filtra a_cmd, alpha_cmd) --------
        a_safe, alpha_safe = qp.cbf_qp_filter_acc(
            ...
            alpha_bounds=(-4.0, 4.0),   # ajusta se precisares
        )

        # integra velocidades a partir das acelerações
        v += a_safe * dt
        w += alpha_safe * dt
        ...
        # limite de yaw-rate compatível com o steering máximo
        kappa_max = np.tan(delta_max) / L
        w_max_speed = abs(v) * kappa_max

        # evita ficar com w impossível para o bicycle
        ...
        # yaw-rate REAL imposto pelo bicycle com o delta aplicado
        w = (v / L) * np.tan(delta)
        ...
        if k % 5 == 0:
            ...
            ax.grid(True)
            ax.set_title(
                f"PP + CBF-QP(acc→bicycle) | Ld={Ld:.2f} | v={v:.2f} | w={w:.2f} | a={a_safe:.2f} | alpha={alpha_safe:.2f} | cte~{cte:.3f}"
            )
