...
def update_track_with_vision(track, cx, cy, t_now, source="vision"):
    ...
    while True:
        ...
        if frame_counter > WARMUP_FRAMES:
            for tag_id, tr in confirmed_tracks.items():
                ...
                if det is not None:
                    cx, cy = det["center"]
                    update_track_with_vision(tr, cx, cy, t, source=det.get("source", "vision"))
        ...
        for tag_id, tr in confirmed_tracks.items():
            ...
            x_out_px = float(x)
            y_out_px = float(y)
            if tr.get("last_source") == "yolo" and tr.get("last_meas") is not None:
                x_out_px = float(tr["last_meas"][0])
                y_out_px = float(tr["last_meas"][1])
                x_m, y_m = px_to_m(x_out_px, y_out_px)
            else:
