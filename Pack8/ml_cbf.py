...
class MLCBFParameterModel:
    ...
    def __init__(
        ...
        self.d_values = np.asarray(d_values, dtype=float)
    ...
    def fit(
        ...
        if d_values is None:
            # Backward-compatible fallback for old callers; new Pack8 training
            # passes D_min explicitly.
            d_values = -scores
    ...
    def _kernel_regression(self, normal_params, values):
        normal_params = np.asarray(normal_params, dtype=float)
        if normal_params.ndim == 1:
            normal_params = normal_params.reshape(1, -1)
        ...
        pred = np.full(normal_params.shape[0], fallback, dtype=float)
        good = denom > 1e-12
        pred[good] = weights[good] @ values / denom[good]
        return pred

    def _predict_d_value(self, normal_params):
        return self._kernel_regression(normal_params, self.d_values)
    ...
    def _fgo_search(self, seed=0, random_samples=2500, iterations=10, population=80):
        ...
        # D_hat is the paper-style feasibility-robustness objective surrogate:
        # the predicted minimum barrier/safety margin observed in an episode.
        # Lower D_hat is less conservative, subject to H(y) >= 0.
        def d_hat(x):
            return float(self._predict_d_value(np.asarray(x).reshape(1, -1))[0])

        def h_hat(x):
            return float(self.svm.decision_function(np.asarray(x).reshape(1, -1))[0])

        start_count = max(1, min(int(population), len(feasible_pool)))
        starts = feasible_pool[np.argsort(self._predict_d_value(feasible_pool))[:start_count]]
        nu_limit = 0.35
        dt = 0.25

        best = starts[-1].copy()
        best_d = d_hat(best)
        best_objective = self._objective(best)[0]
        trace = [[0.0, float(best_d), h_hat(best)]]
        ...
        for start in starts:
            ...
            for iteration in range(1, int(iterations) + 1):
                ...
                d_value = d_hat(y)
                objective = self._objective(y)[0]

                if h_hat(y) >= -1e-9 and d_value < best_d:
                    best = y.copy()
                    best_d = d_value
                    best_objective = objective
                elif objective > best_objective and h_hat(best) < 0.0:
                    best = y.copy()
                    best_d = d_value
                    best_objective = objective

            trace.append([float(len(trace)), float(best_d), h_hat(best)])
    ...
    def save(self, path=DEFAULT_MODEL_PATH):
        ...
        np.savez(
            path,
            model_version=np.array([4], dtype=int),
    ...
    def load(cls, path=DEFAULT_MODEL_PATH):
        ...
        if "model_version" not in data:
            ...
            return cls(
                ...
                d_values=-np.asarray(mean_scores, dtype=float),
                labels=labels,
                svm=svm,
        ...
        return cls(
            ...
            scores=data["scores"],
            d_values=data["d_values"] if "d_values" in data else -data["scores"],
            labels=data["labels"],
            svm=svm,
