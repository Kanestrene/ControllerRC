...
```powershell
.\env\Scripts\python.exe Pack8\train_ml_cbf.py
```

Isto cria:

- `Pack8/ml_cbf_pq_model.npz`
- `Pack8/ml_cbf_training_report.csv`
- `Pack8/ml_cbf_fgo_params.csv`

Para um teste rapido:

```powershell
.\env\Scripts\python.exe Pack8\train_ml_cbf.py --samples 6 --episodes-per-sample 1 --horizon 8 --feasible-min-progress 0.15
```

Para uma volta completa:

```powershell
.\env\Scripts\python.exe Pack8\train_ml_cbf.py --samples 36 --episodes-per-sample 1 --horizon 50
```

## Comparar baseline vs ML-CBF

```powershell
.\env\Scripts\python.exe Pack8\simulate_ml_cbf.py
```

## Correspondencia com o artigo

- No artigo, `p=(p_1,...,p_m)` e `q=(q_1,...,q_m)` parametrizam as funcoes de classe K da HOCBF.
- No Pack8, `m=1`, portanto aprende-se `class_k_p = p_1` e `class_k_q = q_1`.
...
- O objetivo `D` usado pela FGO e `D_min`, isto e, o menor valor observado da margem
  de seguranca do episodio. Ele substitui o uso de `-score` como objetivo da FGO.
