...
def extended_class_k(h, p=2.0, q=1.0):
    """
    Paper-style extended class K function for the first-order CBF:
        alpha_1(h) = p * sign(h) * |h|^q.

    The sign-preserving form keeps the function defined when h < 0, which can
    happen during recovery or because of numerical/learning errors.
    """
    h = np.asarray(h, dtype=float)
    return float(p) * np.sign(h) * np.power(np.abs(h), float(q))
...
def cbf_rows_for_circle_obstacles(x, y, th, obstacles,
    ...
    for obs in obstacles:
        ...
        # CBF: a_v v + a_w w + alpha_1(h) >= 0
        # with alpha_1(h) = p sign(h)|h|^q, as in the paper for m=1.
        beta_h = extended_class_k(
            h_val,
            p=alpha if class_k_p is None else class_k_p,
            q=class_k_q,
        )
        # => -(a_v v + a_w w) <= alpha_1(h)   (formato G u <= h)
        G_list.append([-a_v, -a_w])
        h_list.append(beta_h)
