...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compara Pack6 baseline CBF-QP contra ML-CBF em v,w."
    )
...
def plot_comparison(baseline, learned, path):
    ...
    ax.set_title("Pack6: baseline CBF-QP vs ML-CBF")
...
def main():
    ...
    if not args.model.exists():
        raise FileNotFoundError(
            ...
            "Corre primeiro: python Pack7/train_ml_cbf.py"
        )
    ...
    active = learned.get("last_active_params", {})
    if active:
        print("parametros FGO usados:")
        for key in (
            "class_k_p",
            "class_k_q",
            "alpha",
