...
def _apply_ml_cbf_params(
    ...
    class_k_p = float(selected.get("class_k_p", class_k_p))
    class_k_q = float(selected.get("class_k_q", class_k_q))
    alpha = float(selected.get("alpha", class_k_p))
...
def cbf_qp_filter(u_nom, robot_state, obstacles,
    ...
    G_obs, h_obs = cbf.cbf_rows_for_circle_obstacles(
        ...
        class_k_p=class_k_p, class_k_q=class_k_q
    )
...
def cbf_clf_qp_filter(
    ...
    margin=0.05,
    lookahead_l=0.35,
    alpha=2.0,               # CBF fallback: alpha_1(h)=alpha*h
    class_k_p=None,          # paper notation p_1 for alpha_1(h)
    class_k_q=1.0,           # paper notation q_1 for alpha_1(h)
    eps_clf=1.0,             # CLF
    ...
    (
        ...
        ellipse_ab=ellipse_ab,
        alpha=alpha,
        class_k_p=alpha if class_k_p is None else class_k_p,
        class_k_q=class_k_q,
        margin=margin,
        lookahead_l=lookahead_l,
        barrier_lookahead_l=barrier_lookahead_l,
