...
def score_episode(metrics):
    ...
    mean_abs_cte = float(metrics["mean_abs_cte"])
    ...
    # Reward trajectories that use the available corridor closely, but only
    # while they remain safe. Negative margins are handled by the penalty below.
    safe_clearance = max(0.0, safety_margin)
    #score += 180.0 * np.exp(-safe_clearance / 0.25)
